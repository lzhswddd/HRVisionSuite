    ...
    def test(key:str,img:QImage):
        if key == "measure.calipers":
            ...
            if isinstance(calipers, dict):
                top = calipers.get("top", [])
                left = calipers.get("left", [])
                bottom = calipers.get("bottom", [])
                right = calipers.get("right", [])
                ...
                topResult = topMeasure.run(npImage, 
                ...
                leftResult = leftMeasure.run(npImage,
                                           sigma=param_cfg.get("measure.sigma"),
                                           threshold=param_cfg.get("measure.threshold"),
                                           transition=param_cfg.get("measure.transition"),
                                           select=param_cfg.get("measure.select"))
                bottomResult = bottomMeasure.run(npImage,
                                           sigma=param_cfg.get("measure.sigma"),
                                           threshold=param_cfg.get("measure.threshold"),
                                           transition=param_cfg.get("measure.transition"),
                                           select=param_cfg.get("measure.select"))
                rightResult = rightMeasure.run(npImage,
                                           sigma=param_cfg.get("measure.sigma"),
                                           threshold=param_cfg.get("measure.threshold"),
                                           transition=param_cfg.get("measure.transition"),
                                           select=param_cfg.get("measure.select"))
