# -*- coding: utf-8 -*-
"""主窗口：YOLO 检测窗格（GraphicsView 架构，hrfluentwidgets 现成控件）。

显示数据走「显示槽」共享内存（DisplaySlot 单写单读，头部带通道数——
USB 相机无文件可探测尺寸，读方按 channels 字节直接还原 3D）：
    原图槽：相机进程写；结果图槽：算法进程写（画框后）。
主进程只做读方：定时器按显示状态（勾选）读对应槽，seq 变化才 setImage；
QApplication 懒创建（仅主进程 UI 节点调用；子进程不付 Qt GUI 开销）。
"""
...
class MainWindow(QWidget):
    """YOLO 检测窗格：CameraView + 原图/结果切换 + 检测信息标题。"""
    ...
    def __init__(self, gData, signal_instance, fps_limit: int = 15):
        ...
        self.setWindowTitle("UsbYolo - %s 模式（USB 相机 + YOLO 识别）" % gData.user.run_mode)
        self.resize(1280, 800)

        keys = gData.user.pane_list
        grid = QGridLayout(self)
        for i, key in enumerate(keys):
            row, col = i // 2, i % 2   # 2 列网格（单窗格用第 0 格）
            cell = QWidget()
            v = QVBoxLayout(cell)
            ...
            signal_instance.yoloResult.connect(
    ...
    # ---------- 显示槽读取（DisplaySlot 封装：头部通道数还原 + 未就绪静默） ----------

    def _read_slot(self, key: str, kind: str):
        """读窗格显示槽：返回 (seq, frame) 或 (None, None)（写方未就绪静默跳过）。"""
    ...
    def _make_on_result(self, key: str):
        # 信号带节点 id（如 yolo_1），窗格 key 是 pane id（如 pane_yolo_1）——
    ...
    def _refresh(self):
        for key, (view, checkbox, imgs) in self.user.windows.items():
            ...
            res_str = ("-"
                       if not r else
                       "%d 个目标 推理%sms" % (r.get("count", 0), r.get("time_ms", 0)))
            if r and r.get("detections"):
                d = r["detections"][0]
                res_str += "  [%s %.2f]" % (d["cls"], d["conf"])
