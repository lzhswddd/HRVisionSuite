# -*- coding: utf-8 -*-
"""VmDemo 全局数据：GlobalData(GlobalDataBase) + UI 信号。
    ...
    def __init__(self):
        ...
        if os.path.exists(spec_path):
            ...
            self.pipeline_spec = make_pipeline_spec(
                {"grab_0": {"flow": "camera", "vm_mode": "sim",
                            "sim_source": "videos/vm1.avi",
                            "vm_solution": "D:/Python/frame/ProcessTest/Flow/test.sol"}},
                {"algo_1": {"flow": "algo", "vm_mode": "sim",
                            "vm_solution": "D:/Python/frame/ProcessTest/Flow/test.sol"}})
        self.fps_limit = 15
...
class SignalProgram(QObject):
    ...
    大图/帧数据走显示槽（共享内存），信号只传轻量消息（结果摘要/状态）。
    """

    vmResult = Signal(str, dict)   # (algo_key, {"ok", "vm_comm", ...})
