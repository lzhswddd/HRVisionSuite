# -*- coding: utf-8 -*-
"""主窗口：VM 处理窗格（CameraView）+ VM 模式/通信状态栏。
...
class MainWindow(QWidget):
    """VM 处理窗格：CameraView + 原图/结果切换 + VM 模式/通信状态标签。"""
    ...
    def __init__(self, gData, signal_instance, fps_limit: int = 15):
        ...
        self._status: QLabel = QLabel("等待 VM 图像...")
        _APP = self
        self.setWindowTitle("VmDemo - %s 模式（VM 采图 → 算法 → 通信结果）"
                            % gData.user.run_mode)
        ...
        for i, key in enumerate(keys):
            ...
            signal_instance.vmResult.connect(
    ...
    def _make_on_result(self, key: str):
        ...
        def on_result(algo_key, result):
            ...
            self._status.setText("VM 结果 %s（%sms）| %s" % (
                st, result.get("time_ms", 0.0), result.get("vm_comm", "未发送")))
        return on_result
    ...
    def _refresh(self):
        for key, (view, checkbox, imgs) in self.user.windows.items():
            ...
            res_str = ("-"
                       if not r else
                       ("OK" if r.get("ok") else "NG") + " mode=" + str(r.get("mode", "")))
